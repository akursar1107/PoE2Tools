...
async function main() {
  ...
  const skillLines = skills.map(([key, gem]) => {
    ...
    buildId: ${JSON.stringify(key.startsWith('Metadata/') ? key : null)},
...
`;
  writeFileSync(join(ROOT, 'data/skills.mjs'), skillsOutput);
  console.log(`Wrote data/skills.mjs (${skills.length} gems)`);

  // ── Write data/supports.mjs ───────────────────────────────────────────────
...
`;
  writeFileSync(join(ROOT, 'data/supports.mjs'), supportsOutput);
  console.log(`Wrote data/supports.mjs (${supports.length} gems)`);
